...
export default function Seperator({ width, className, fade, color, opacity }: seperatorProps) {
    return (
        <div className={className ? `relative ${className}` : 'relative'}>
            <hr className={`bg-neutral-00 opacity-1100 my-2 h-0.5 border-t-0 dark:opacity-50 ${fade ? 'bg-gradient-to-r from-transparent to-white' : ''} ${className}`} style={{ width: `${width}%`, backgroundColor: color, opacity: opacity }}></hr>
        </div>
    );
}
