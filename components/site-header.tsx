...
const MenuPill = ({ pulse, pulseClass, children }: MenuPillProps) => {
  return (
    <Pill
...
const navigationMenu = [
  ...
  { label: "Issues", icon: Icons.todo, href: "issues" },
  {
    ...
    badge: (
      <>
        <MenuPill>WiP</MenuPill>
        <MenuPill>Buggy</MenuPill>
      </>
    ),
  },
  {
    ...
    badge: (
      <>
        <MenuPill>WiP</MenuPill>
        <MenuPill>Experimental</MenuPill>
      </>
    ),
  },
  {
...
const SiteHeader = ({ children }: { children?: React.ReactNode }) => {
  ...
  return (
    <>
      <AnimatedElement
        className="hidden max-hw-[97vh] flex-col text-blacktheme dark:text-accent sm:flex h-screen sticky top-8"
        as="aside"
        opacity={0}
        duration={0.5}
        ease="EASE_IN"
