...
export default function ProbabillityCalculator() {
  ...
  const calculateProbability = () => {
    if (useVariant2 && (totalTiles !== 49 || totalBombs !== 5)) {
      alert(
        "Invalid Input! For variant 2, the total tiles should be 49 (7x7 grid) and total bombs should be 5.",
      );
      return;
    }
    ...
    if (tilesToOpen > 0) {
      ...
      const multiplier = useVariant2
        ? multiplierValues.variant2[multiplierIndex]
        : multiplierValues.variant1[multiplierIndex];
  ...
  return (
    <Card>
      ...
      <div>
        <Button variant="outline" onClick={calculateProbability}>
