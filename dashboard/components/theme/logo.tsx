...
const LogoIcon = ({ color = '#fff', width = 'auto', height = '48' }: LogoProps) => (
    <svg
        width={width}
        height={height}
        x="0"
...
function Letters({ color = '#fff', width = 'auto', height = '48' }: LogoProps) {
    return (
...
export default function Logo({ color = '#fff', width = 'auto', height = '48', hasUrl = true, hasIconOnly = false, ...rest }: LogoProps) {
    const svgContent = (
        <svg
            width={width}
            height={height}
        ...
        >
            ...
            {!hasIconOnly && <Letters color={color} width={width} height={height} />}
        </svg>
    );

    if (hasUrl) {
        return <Link href="/">{svgContent}</Link>;
    }
